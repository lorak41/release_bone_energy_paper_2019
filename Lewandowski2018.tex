...
elastic deformation is mapped from the current material to spatial domain,
...
coordinates, ${ \boldsymbol {\rm \upchi}}$, on to the material coordinates,
...
\end{equation}
Moreover we define the right Cauchy-Green deformation tensor $\boldsymbol
{\rm C}$ are denoted as follows:
\begin{equation}
...
Note that the density is current material filed, defined
as
\begin{equation}
	\rho = \rho(\mathbf{X},t) 
	= \rho\left(
...
Following Kuhl and Steinmann~\citep{kuhl2003computational}, we assume that
the rate of change of the time-dependent material density is in equilibrium
with the mass flux evaluated as:
\begin{equation}
\frac{\partial\rho}{\partial t}  + \dot{\mathbf{W}} \cdot  \nabla_\mathbf{X} \rho = 
\nabla_{\boldsymbol {\rm X}} \cdot \mathbf{R} + \mathcal{R}_0
\label{eq:mass_balance}
\end{equation}
where $\rho$ is mass density, $\mathbf{R}$ is the mass flux and
$\mathcal{R}_0$ is the locally created mass. The term on the right hand side
is material time derivative associated which evolving current material
configuration, as result of changing topology accounting for crack
propagation. However, if it is assumed that the mass flux $\mathbf{R}$ is zero,
such that only the local mass source $\mathcal{R}_0$ contributes to the
changes in density. 

Description of remodelling in the context configurational mechanics has
theoretical and practical implications, the density field has to be
...
bone remodelling, i.e. both are separated by time scales, both problems,
...
of densities in current material configuration and the necessity for
regularity of density field (as will emerge in the following equations), we
choose to approximate bone density using MWLS approximation on fixed
reference configuration. That enables us to map bone density and its
gradients of bone density on finite element mesh integration point, and
...
The crack surface is denoted as $\Gamma$ with a crack front $\partial
...
consisting of a solid deformable body with an imperfection, i.e. crack.
...
to spatial and material velocities we get local conservation laws. The
spatial conservation law of linear momentum balance, for any point inside the
body, is expressed as follows:
\begin{equation} \label{eq:linear_momentun}
...
\end{equation}
where $\partial\mathcal B_t^\sigma$ is where tractions are applied. 
Therefore, the first term on the right-hand side in
Eq.~(\ref{eq:crack_local_gauss}) vanishes. However, the material conservation
...
form first thermodynamic law which needs to be satisfied at every material
point in current material configuration to make the body in equilibrium.

Noting that, the surface $\mathcal C$ (Figure~\ref{fig:frac_crack_con})
...
\end{equation}
This equation represents the equilibrium condition for the crack front. From
the above equation, two important observations can be concluded. One, crack
front is in equilibrium where the crack is not propagating, i.e. material
velocity at the crack front is zero, or crack front is propagating and driving
force is in equilibrium with an material force. Two, front equilibrium takes
a similar form to presented in \citep{kaczmarczyk2017energy}, that as will be
shown later justifies the use of the Griffith criterion, originally proposed
for homogenous materials.

Note that in this work we take into account dissipation energy associated
only with topology evolution, for materials like a bone that could be further
extended to dissipation associated with crack opening, i.e. cohesive effects. 
However above equations will be still valid.

\subsection{Constitutive equations}

\subsubsection{Fracture process}

Fracture process takes place in a direction which results in energy
dissipation, and in this work, we are limiting yourself only to crack growth.
Do not consider the healing process or additional dissipation due to crack
opening as observed in quasi-brittle materials. That could be in future
extended to there processes, associated with crack opening, i.e. accounting
for finite fracture zone. At that point, our brittle model assumes that
trabecular element break at once at the crack front only.

It can be noted that equation (\ref{eq:crack_local_first_law}) has
bifurcation the point, i.e. more that one solution at equilibrium; the crack
can or not propagate. One way to distinguish between those two answers is by
supplementing equations by Griffith criterion as is shown in
\citep{kaczmarczyk2017energy}, or any other appropriate for cyclic loading.
The crack will propagate if crack release energy is equal to material
toughness and not if is lower than it. The state than crack release energy is
larger than material toughness is the inadmissible state.

Another issue is the direction of the fracture process itself, that is
constrained by second thermodynamic law, which is not straightforward for
open leaving system. However with the assumption that fracture takes place
relatively fast comparing to the process of remodelling, no healing takes
places, one can write dissipation at the crack front has to be positive
\begin{equation}
	\mathcal{D} = \gamma \dot{\mathbf{W}} \cdot \mathbf{A}_{\partial\Gamma}
\end{equation}
From all possible crack propagation direction, one can choose that one which
maximises discussion of energy which leads to the equation
\begin{equation}
	\gamma \mathbf{A}_{\partial\Gamma} = \mathbf{G}
\end{equation} 
where the magnitude of $\mathbf{G}$ can be interpreted as crack release
energy $g$, in crack propagation direction $\mathbf{A}_{\partial\Gamma}$;
	\begin{equation}
	2 g = \mathbf{G} \cdot \mathbf{A}_{\partial\Gamma}
\end{equation}

Note, the use of well-established stress intensity factors in case of
heterogeneous material is not applicable since that demands the existence of
an analytical solution for stress filed in the vicinity of the crack front,
independent on arbitrary distribution of density. Similarly, use of
...
Also, note that we present equations in general large displacements and large
strains setting, and presented formulation is valid crack release energy
in such general case.
...
the rate of the remodelling process with units~$[\rm{s/cm^2}]$. As shown
in~\citep{Waffenschmidt2012}, it is beneficial to prescribe an upper and
lower bound for bone density, thereby avoiding spurious or non-physical
values. In this paper, the constant $c$ is replaced by a bell function
defined as:
\begin{equation}
\begin{aligned}
...
\label{eq:free_energ}
\end{equation}
where $\psi^{\rm {neo}}_0$ is the Helmholtz free energy chosen to be of
Neo-hookean type defined as: %where Helmholtz free energy $\psi$ was chosen
...
\subsubsection{Remarks on physical equation implementation}

\section{Discretisation}

We were solving bone remodelling problem and calculation of crack release
forces. In this paper, we do not study crack propagation itself. Since the
issue of bone remodelling takes places in longer time scales, and demand
problem discretisation with different mesh than the problem of calculation of
crack release energy, density from one mesh to another is mapped using MWLS
approximation. Mapped density filed has higher regularity than one
approximated directly on the mesh. That feature of higher regularity of MWLS
...
In the context of finite element method, we first approximate fields in spaces,
following fields
\begin{eqnarray}
	\rho^h(\pmb\chi,t) = \pmb\Phi({\boldsymbol {\rm {\upchi}}}) {\tilde{\pmb\rho}}(t) \\
	\mathbf{ X^h}(\pmb\chi,t) = \pmb\Phi({\boldsymbol {\rm {\upchi}}}) \tilde {\mathbf{ X}}(t), 
	\quad \mathbf{x^h}(\pmb\chi,t) = \pmb\Phi({\boldsymbol {\upchi}}) {\tilde{\mathbf{x}}}(t) \\
	\mathbf{ W^h}(\pmb\chi,t) = \pmb\Phi(\upchi) {\dot { \tilde {\boldsymbol {\rm W} } }}(t), 
	\quad \mathbf{w^h}(\pmb\chi,t) = \pmb\Phi({\boldsymbol {\upchi}}){\dot { \tilde {\boldsymbol {\rm w} } }}(t)
\end{eqnarray}
...
following the work of Ainsworth and Coyle~\citep{Ainsworth2003}. In addition
smoothed density filed approximated by MWLS shape functions
\begin{equation}
	\rho^{h,\textrm{mwls}}(\mathbf{X},t) = \Phi^\textrm{mwls}(\mathbf{X}) 
	\tilde{\pmb\rho}^h(\Xi(\pmb\chi),t) 
\end{equation}
Note that shape functions $\Phi^\textrm{mwls}(\mathbf{X})$ are evaluated at
current material points, and are a partition of unity. That this
approximation is independent on changing the material configuration, i.e.
changing mesh, and degrees of freedom $\tilde{\pmb\rho}^h(\Xi(\pmb\chi),t)$ are
values of density at evaluated points.

\subsection{Bone remodeling problem in week form}

Since here we propose a staggered approach, we fix material configuration
such that
\begin{equation}
	\tilde {\mathbf{ X}}(t)	= \tilde {\pmb\chi} =  \textrm{const},
	\;\textrm{and}\,
	\dot{\tilde {\mathbf{X}}}(t) = \dot{\tilde {\mathbf{W}}}(t) = 0
\end{equation}
where $\tilde {\pmb\chi}$ is vector of nodal positions. The semi-discrete
form of equations (\ref{eq:mass_source}) and (\ref{eq:linear_momentun}) takes
the form of residuals
\begin{equation}
	\left\{
	\begin{split}
		\mathbf{r}^\rho(\tilde{\pmb\rho}(t), \tilde{\mathbf{x}}(t)) =
		\int_{\mathcal{B}^h_t} \pmb{\Phi}^\textrm{T}\dot{\rho}^h(\pmb\chi,t) 
		\textrm{d}V 
		+
		\int_{\mathcal{B}^h_t} \nabla_\mathbf{X} \pmb{\Phi}^\textrm{T} 
		\mathcal{R} \pmb{\Phi}^\textrm{T} 
		\textrm{d}V
		-	
		...
		\mathbf{r}^\textrm{external} 
		\textrm{d}S	
		= \mathbf{0}\\
		\mathbf{r}^x(\tilde{\pmb\rho}(t), \tilde{\mathbf{x}}(t)) =
	...
	\right.
\end{equation}
where $\mathbf{r}^\textrm{external}$ is influx of mass through boundary,
$\mathbf{f}^\textrm{external, remodelling}$ are averaged long term forces
mimicking mechanical load if the bone over long time period, rather scale of
weeks than days, and $\mathcal{R}$ is mass conductivity. After expanding
residuals in Taylor series, and truncating after linear term, i.e.
linearization, we get algebraic system in semi-discrete form
\begin{equation}
	\left[
		\begin{array}{c}
			\mathbf{r}^\rho(\tilde{\pmb\rho}_i(t), \tilde{\mathbf{x}}_i(t))\\
			\mathbf{r}^x(\tilde{\pmb\rho}_i(t), \tilde{\mathbf{x}}_i(t))
		\end{array}
	\right]
	+
	\left[
	...
	\left\{
		\begin{array}{c}
		\delta\dot{\tilde{\pmb\rho}}_{i+1}(t)\\
...
\mathbf{M}_{\rho\rho} = 
\left.
\frac{\partial \mathbf{r}^\rho}{\partial \dot{\tilde{\pmb\rho}}}
\right|_{(\tilde{\pmb\rho}_i(t),\tilde{\mathbf{x}}_i(t))},\,
\mathbf{K}_{\rho\rho} = 
\left.
\frac{\partial \mathbf{r}^\rho}{\partial \dot{\tilde{\mathbf{x}}}}
\right|_{(\tilde{\pmb\rho}_i(t),\tilde{\mathbf{x}}_i(t))},\,
\textrm{add rest}
\end{split}
\end{equation}
and 
\begin{equation}
\tilde{\pmb\rho}_{i+i}(t) =
\tilde{\pmb\rho}_{i}(t) + \delta\tilde{\pmb\rho}_{i+i}(t),\;
	\tilde{\mathbf{x}}_{i+i}(t) =
	\tilde{\mathbf{x}}_{i}(t) + \delta\tilde{\mathbf{x}}_{i+i}(t)
\end{equation}
where $(\dot)_i$ is quantity at Newton iteration $i$. Finally we discretise
above semi-discrete problem in time using implicit Euler scheme 
\begin{equation}
	\dot{\tilde{\pmb\rho}}_{i+i}^{n+1}(t) =
	\frac{
	\tilde{\pmb\rho}_{i+i}^{n+1}-\tilde{\pmb\rho}^{n}
	}{\Delta t}
\end{equation}
where $\Delta t$ is length of time step, and $n$ is time step number.

Note that density is approximated one polynomial than spatial positions in
order to get stabile solution without oscillations.

\subsection{Elastic problem with the crack}

The problem of bone remodelling and crack propagation or calculation of crack
release energy has different boundary conditions and geometry. First, is used
to simulate bone remodelling under long applied effective loads and without
initial crack, second, is the effect of short load or extreme cycling load
with initial crack. That creates the necessity of running simulation on two
different meshes, which are tailored for two different types of problems. As
a consequence, we express approximated Piola stress tensor for bone
remodelling in the form
\begin{equation}
	\mathbf{P}^\textrm{h} = 
		\mathbf{P}(\mathbf{F}^\textrm{h}, \rho^h)
\end{equation}
whereas for elastic problem with the crack approximated Piola stress tensor is
\begin{equation}
...
scans, and run bone remodelling analysis, or introduce crack directly run
elastic analysis to calculate stress release energy. In each case, initial
density would be approximated using MWLS and then projected on current
material configuration.

To conclude residual for elastic problem leading to calculate stress release 
energy is
\begin{equation}
	\mathbf{r}^x(\tilde{\pmb\rho}(t), \tilde{\mathbf{x}}(t)) =
	\int_{\mathcal{B}^h_t} \nabla_\mathbf{X} \pmb{\Phi}^\textrm{T}
	...
	= \mathbf{0}
\end{equation}
where $\mathbf{f}^\textrm{external, elastic}$ is external force which lead 
immediately to fracture or cycling loading which directly lead to fracture. 
Since in this paper, we are limiting ourselves to calculation crack release
energy only, the nodal internal material forces on front crack nodes are
...
\begin{equation}
	...
	+
	\int_{\mathcal{B}^h_t}
		\pmb{\Phi}^\textrm{T} {\pmb\Sigma}^{h,\textrm{mwls}} 
		\left(
			\frac{\partial 
	...
	\textrm{d}V	
\end{equation}
where $\mathbf{f}^\textrm{material,internal,front}$ is vector of degrees of
freedom only on crack front nodes and integration takes only place on
tetrahedrons adjacent to crack front nodes, similarly to integration crack
area tangent vectors matrix presented in the next subsection.

 \subsection{Material problem for crack release energy}

 Note that we evaluated nodal material forces 
 $\mathbf{f}^\textrm{material,internal,front}$ at
 the crack front nodes. Also, the dimension of material nodal force is
 $[J/m]$, but Griffith cracks release energy should be in units $[J/m^2]$.
 That discrepancy disappears in-plane stress analysis, in that case, material
 forces are divided by the thickness of the body. In that case magnitude of
 nodal material forces at the crack front node is value $J$ -integral. In the
 case of 3d analysis to remove that unit discrepancy in units some authors
 [cite Mieche] divide half of the adjacent edges. However, that gives the
 correct value for crack release energy only for straight crack fronts.

Let surface of domain approximation of body at current material configuration
is given by ${\partial\mathcal{B}^h_t}$. Non overlapping triangles approximating, 
covering whole domain ${\partial\mathcal{B}^h_t}$ are represented by 
\begin{equation}
	\mathcal{T} := \left\{ 
		\mathcal{T} \in {\partial\mathcal{B}^h_t} : 
		\mathcal{T}_i \cap \mathcal{T}_i  = \O\;
		\textrm{and}\,
		{\partial\mathcal{B}^h_t} = \sum_i^N \mathcal{T}_i 
	\right\}
\end{equation}
where crack front is approximated by edges $\Gamma^h$, defined in similar way
like triangles, but only one dimension lower. With above at hand, we can
define the triangulated domain, which triangles are part of body boundary and
adjacent to crack front, as follows
\begin{equation}
 S^h_\Gamma :=
 \left\{
 ...
 \right\}
\end{equation}
Next, we can define a matrix of  crack tangent vectors as follows
\begin{equation}
\mathcal{C}^\textrm{front} = 
\int_{S^h_\Gamma}
\pmb{\Phi}^\textrm{T} 
\frac{\partial \mathbf{A}^h_{\Gamma}}{
...
\left\| 
\epsilon_{ijk}
\frac{\partial \Phi^\alpha_m}{\partial \xi_i}  
\frac{\partial \Phi^\beta_n}{\partial \xi_j} 
\tilde{X}^\alpha_m
\tilde{X}^\beta_n
\right\|
\textrm{d}S
\end{equation}
where $\alpha, \beta \in \{0, \dots, N\}$ are number of base functions, and
$i,j,k,l,m,n \in \{0,1,2\}$ are material indices. The total number of degrees
of freedom on element is $3*(N+1)$. $\pmb\epsilon$ is Levi-Civita symbol.

The nodal crack release energy $\mathbf{g}^\textrm{h}$ can be interpreted as
lagrange multipliers constraining crack area increment, where by exploiting
projection matrices, can be expressed exquisitely by $\mathcal{C}^\textrm{front}$
and $\mathbf{f}^\textrm{material,internal,front}$ as follows
\begin{equation}
	\mathbf{g}^\textrm{h}
	= 
	\left[
	\left(\mathbf{C}\mathbf{C}^\textrm{T}\right)^{-1}
	\mathbf{C}
	\right]
	\mathbf{f}^\textrm{material,internal,front}	
\end{equation}
see details for projection matrices here [cite Ainsworth essential]. Note
that dimension term in brackets is $1/m$ which multiplied by dimension
of material force $[J/m]$, give dimension of crack release energy, i.e.
$J/m^2$. Also, advantage of presented approach is that crack release 
...
out of the scope of this peeper. 

\subsection{Comments about implementation}

\textcolor{red}{Now you have ADOL-C and analytical, you can comment on
performance of both and moved.} 
The coupled system of nonlinear equations (mass
balance~(\ref{eq:mass_balance}) and momentum
balance~(\ref{eq:momentum_balance})) are discretised and solved iteratively
using the Newton-Rephson method for the displacements and density. The
tangent stiffness is derived by means of automatic differentiation using the
ADOL-C library~\citep{Walther2009}. Spatial discretisation is achieved using
...
is achieved using the backward Euler method. Application of the proposed
...
In this study, crack propensity is evaluated by computing nodal configurational forces $\mathbf{\tilde G}^h$ (see Eq. (\ref{eq:crack_material_force_dis})) that are dependent on the gradient of elastic energy over change of density, $\mathbf{B_x}^{\rm T} \rho$. 
...
Gradients form MWLS, on the contrary, are smooth, which can be of great importance when the mapping is used in crack propagation analysis since it strongly influences the direction of a crack. 
...
%	\includegraphics[width=0.7\linewidth]{Figures/graphs/plate_conv_singularity.png}
	\input{Figures/tikz/plate_conv_singularity.tex} 
...
Since the stress intensity factor loses its meaning in the case of heterogeneous bodies, correct validation of configurational forces still remains an open question. 
...
%	\includegraphics[width=0.7\linewidth]{Figures/graphs/covergence_FDM.png}
	\begin{tikzpicture}
		...
		\node at (5,-1.5) {	\includegraphics[width=2cm]{Figures/tikz/gradient.png}};
		\end{tikzpicture}
