...
bifurcation point, i.e. there are more that one solution at equilibrium, where the crack either
...
constrained by second law of thermodynamics, which is not straightforward for
open-system. However, with the assumption that fracture takes place
...
processes of bone remodelling takes place at longer time scales than fracture,
...
order to get stable solution without oscillations.
...
scans, run bone remodelling analysis and subsequently introduce crack and calculate the strain 
release energy. It is also possible to load densities directly onto mesh with a prescribed fracture.
...
\begin{equation}
	...
	= \mathbf{0}
\end{equation}
where $\mathbf{f}^\textrm{external, elastic}$ is an external force which leads 
immediately to fracture or cycling loading which directly leads to fracture. 
...
area tangent vectors matrix presented in the next subsection.
 ...
 The nodal material forces 
 $\mathbf{f}^\textrm{material,internal,front}$ are evaluated at
 the crack front nodes. Moreover, the dimension of material nodal force is
 ...
 nodal material forces at the crack front node is the value of J-integral. In the
...
of material force $[\rm{J/m}]$ gives dimension of crack release energy, i.e.
$[{\rm{J/m^2}}]$. Furthermore, the main advantage of the presented approach is that crack release 
...
\end{figure}
It is apparent from the plot that with increasing levels of refinement the error in release energy is converging down to 0.3\%. It is worth noting that for homogeneous case similar accuracy is attained.  Achieving higher precision with FDM is difficult due to accumulation of truncation, approximation and discretisation errors.
