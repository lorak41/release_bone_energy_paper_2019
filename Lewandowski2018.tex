...
Density is a field that evolves in the current material configuration and defined as
\begin{equation}
	\rho = \rho(\mathbf{X},t) 
	= \rho\left(
...
configuration, as result of changing topology resulted from crack
...
Description of remodelling in the context of configurational mechanics has
...
point in current material configuration to satisfy equilibrium.
...
\end{equation}
The last equation represents the equilibrium condition for the crack front. From 
Eq.~(\ref{eq:crack_local_first_law}), two important observations can be made. 
...
force is in equilibrium with material forces. Secondly, front equilibrium takes
...
Note that in this work we take into account dissipation of energy associated
only with topology evolution. For materials like bone, dissipative mechanism 
...
In the present work, propensity of fracture process evolution 
in directions that result to maximal energy dissipation is 
considered only for mechanisms related to perfectly brittle crack growth.
...
bifurcation point, i.e. there are more that one solution at equilibrium, where the crack
can or not propagate. One way to identify the correct solution between those two answers is by
supplementing equations by Griffith criterion as proposed in
...
\end{equation}
From all possible crack propagation directions, one can choose the one which
maximises dissipation of energy which leads to the following equation
\begin{equation}
...
Moreover, it is important to stress that this framework was formulated within 
...
All previously presented framework involved solving remodelling problem and
and calculation of crack release forces. 
In this paper, crack propagation is not concerned. Since the
processes for bone remodelling take place at longer time scales than fracture,
...
Mapping of density field from the first to the second mesh is achieved using MWLS  
...
In the context of finite element method, we first approximate fields in current material and current spacial spaces,
following fields
\begin{eqnarray}
...
Having density field evaluated at $\mathbf{X}$, approximation is independent of changes of the material configuration, i.e.
changing mesh, and degrees of freedom $\tilde{\pmb\uprho}^h(\Xi({\boldsymbol {\rm \upchi}}),t)$ are
values of density at evaluated points.

\subsection{Bone remodeling problem in weak form}
...
\end{equation}
where $(\cdot)_i$ is quantity at Newton iteration $i$. Finally we discretise
...
Note that density is approximated one polynomial less than spatial positions in
...
To solve the staggered coupled problem, initially bone remodelling under 
long-term effective loads applied without initial crack is simulated. Subsequently, an 
...
scans and either introduce crack directly and run elastic analysis to calculate stress release energy or
run bone remodelling analysis first and s, or. In each case, initial
...
triangles, but with one dimension lower. With the above at hand, we can
define the triangulated domain whose triangles are part of body boundary and
adjacent to crack front as:
\begin{equation}
 S^h_\Gamma :=
 \left\{
...
\right\|
\end{equation}
where $\alpha, \beta \in \{0, \dots, N_{\rm{base}}\}$ are number of base functions,
...
Therefore, multiplication of $\mathbf{Z}^{\rm{front}}$ with nodal crack release energy ${\tilde{\mathbf{g}}}^h$ results to
evaluation of crack material forces:
...
Combining Eqs.(\ref{eq:mat_int_crack}) and (\ref{eq:mat_int_front}) equilibrium at the crack front can be evaluated through the residual below
...
Multiplying Eq.~(\ref{eq:residual_crack}) on both sides with $\mathbf{Z}^\textrm{front}$ and solving for ${\tilde{\mathbf{g}}}^h$ the expression below is obtained
...
Furthermore, dimension term in square brackets on LHS of Eq.~(\ref{eq:gc_lagrange}) is $[\rm{1/m}]$ which multiplied by dimension
of material force $[\rm{J/m}]$, give dimension of crack release energy, i.e.
$[{\rm{J/m^2}}]$. Also, advantage of presented approach is that crack release 
...
balance~Eq.~(\ref{eq:linear_momentum})) are discretised and solved iteratively
using the Newton-Rephson method for the displacements and density.

For this, two approaches have been implemented regarding evaluations of tangent stiffness,
i.e by either analytically evaluating the higher dimentional derivatives of the residuals of $\mathbf{r}_{x}$ and $\mathbf{r}_{\rho}$ as 
presented in Eq.~(\ref{eq:tangent_remodelling}) or by means of automatic differentiation using the ADOL-C 
library~\citep{Walther2009}. At this point, it is interesting to note that analyses using automatic 
differentiation is about 25~$\%$ slower than the analytical formulation.
...
Displavement $u^{(2)}$ is the degree of freedom associated to the 2-nd order shape functions $N_2$ and is not related to a physical node, fictitious location of the degree of freedom can be achieved through multiplication of shape function with coefficient $\kappa$. 
