...
In the context of bone mechanics, the constitutive law for the mass source $\mathcal{R}_{0}$ is commonly a form of the relation proposed by Harrigan and Hamilton~\cite{Harrigan1993}. 
...
\begin{equation} \label{eq:extern_pow}
	\mathscr{P}:=\int_{\partial \mathscr{B}_{t}} \dot{\mathbf{u}} \cdot \operatorname{td} S=\int_{\partial \mathscr{B}_{t}}\left\{\dot{\mathbf{w}} \cdot \mathbf{t}-\dot{\mathbf{W}} \cdot \mathbf{F}^{\mathrm{T}} \mathbf{t}\right\} d S
\end{equation}
...
$i,j,k,p,r \in \{0,1,2\}$ are material indices and $\boldsymbol{\upepsilon}$~is the Levi-Civita tensor. 
