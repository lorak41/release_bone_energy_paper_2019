...
directly approximated on the finite element mesh. Full details are given in \ref{sec:mwls}. 
...
However, by observing the range of densities obtained, it is evident that there is slow convergence to the same solution as Case~1 (Figure~\ref{fig:mc3_density}). 
\begin{figure}[h!]
\centering
...
For Case~3, a more moderate value for the exponent in the bell function was chosen along with a narrower density range than for Case~2 (see Table~\ref{fig:mc3_biol_eq}).
With a much lower value for exponent, $b$, the algorithm no longer has problems converging. 
Reducing the range between the upper and lower bounds of density has a significant impact on the results - Figure~\ref{fig:density_bell2}.
...
\subsection{Moving Weighted Least Squares (MWLS) mapping examples}
\label{sec:mals_mapping}
Validation of the implementation of the MWLS method (described in~\ref{sec:mwls}) is presented via two examples.
The first involves the mapping of an analytical scalar field onto the nodes of a mesh of a prism.
...
An arbitrary analytical field ($f({\boldsymbol {\rm x}})=x + y^2 + z^3 $) %(simulating CT scan data). 
is mapped onto the mesh nodes of the prism (Figure~{\ref{fig:mwlsprism}a}) using the proposed MWLS procedure.
...
The approximated field data, with its gradient, is saved on corresponding nodes as demonstrated in Figure~\ref{fig:mwlsprism}c for a second-order approximation. 
...
The error is evaluated for three cases of different orders of approximation: constant, first-order and second-order. 
...
The maximum error for a first and second-order are $10^{-2}$ and $10^{-4}$, respectively. 
These results are satisfactory for the application of mapping data fields onto the mesh and indicates that the approach has been correctly implemented.
\begin{figure}[h!]
	\centering
...
\end{figure}
Results of the mapping procedures are presented in Figure~\ref{fig:mwlsmapping_cross}. Comparison between the proposed MWLS method and the standard Least Squares (LS) method is shown in Figure~\ref{fig:mwlsmappingcomparisons} for both the density field and the density gradient field.
\begin{figure}[h!]
	\centering
...
Density gradients resulting from the MWLS approximation are smooth, as required for the fracture propagation analysis.
