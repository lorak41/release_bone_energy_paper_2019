...
Performance of the framework is demonstrated using numerical simulations for bone adaptation and subsequent crack propagation, including consideration of an equine 3rd metacarpal bone. The degree of bone adaption is shown to influence both the risk of fracture and the resulting crack path. 
...
directly approximated on the finite element mesh. Full details are given in \citep{karol_lewandowski_moving_2019}.  %LINK TO ZENODO
...
All data for these numerical examples, including meshes and results, are available online [REF - Author, DOI, Repository].
...
\begin{figure}[h]
	...
	\begin{subfigure}[b]{0.475\textwidth}
		\begin{tikzpicture}
			...
			\node at(0.5,1.0){ \includegraphics[width=4.5cm]{Figures/tikz/mc3_density_fig.png}};
	...
	\begin{subfigure}[b]{0.475\textwidth}  
		\begin{tikzpicture}
			...
			\node at(1.5,1.0){ \includegraphics[width=2cm]{Figures/tikz/density_bell1_imag.png}};
			% \node at(3,3){                    	\def\svgwidth{5cm} 	\input{Figures/scale_bars/scale_bar_density_rainbow.pdf_tex}  } ;
			\end{tikzpicture}
	...
	\begin{subfigure}[b]{0.475\textwidth}   
		...
		\begin{tikzpicture}
			...
			\node at(1.5,1.0){ \includegraphics[width=2cm]{Figures/tikz/density_bell2_imag.png}};
			% \node at(3,3){                    	\def\svgwidth{5cm} 	\input{Figures/scale_bars/scale_bar_density_rainbow.pdf_tex}  } ;
			\end{tikzpicture}
		\caption{ Case 3. }    
	...
	\begin{subfigure}[b]{0.475\textwidth}   
		\begin{tikzpicture}
			\node at(0,0){ } ;
			\node at(3,4.5){  \def\svgwidth{4cm} 	\input{Figures/scale_bars/scale_bar_density_rainbow.pdf_tex}  } ;
		\end{tikzpicture}
		% \caption{  }    
	\end{subfigure}
	\caption{ Change in overall mass of the bone over time for 3 cases (see Table~\ref{tab:three_cases}). Density distribution contours in 3rd metacarpal bone at five snapshots in time in (a) and at the last converged step for b) and c).} 
...
% I DELETED MWLS MAPPING EXAMPLES (KAROL)
...
\begin{center}
\begin{tabular}{c}
{\def\svgwidth{15cm} \input{Figures/InfSlab.pdf_tex}}\\
\end{tabular}
...
\begin{figure}[h]
	...
	\begin{minipage}{.45\textwidth}
		...
		\begin{tikzpicture}
			...
			% \node at (-3.4,-0.9)   {		\def\svgwidth{1.8cm}	 	\input{Figures/tikz/plate_conv_no_sing_fig.pdf_tex} };
			\node at (-1.2,-1.1){$0.50\%$};
			\node at (0,0) {};
			\end{tikzpicture}
			\caption{Convergence plot for stress intensity factor $K_{\rm I}$ Relative rror (\%) versus no. of DOF (log10) and deformed shape of the plate (bottom left).}
	...
	\end{tikzpicture}
	\caption{Convergence plot for stress intensity factor $K_{\rm I}$ Relative error~(\%) versus no. of DOF (log10).}
	\label{fig:plate_conv_singularity}
	\end{minipage}
\end{figure}
Based on the results in Figure~\ref{fig:plate_conv_singularity}, it is evident that using singularity elements improves the convergence rate significantly and lowers the error by an order of magnitude, from 0.50\% down to 0.028\%. 
However, it can also be seen from Figure~\ref{fig:plate_conv_singularity} that for each combination of p-refinement, the error increases with further refinement after it reaches the minimum value. 
...
As expected, configurational forces are induced at the crack tip under load and, as explained in Section \ref{sec:release_energy}, these forces are influeced by the non-uniform density distribution. 
...
\begin{figure}[h]
	...
	\caption{The resulting density distribution from the bone adaptation analysis is approximated directly onto integration points of the mesh used for fracture analysis using MWLS method described in~\citep{karol_lewandowski_moving_2019}. The cutting circular plane used to create an initial crack is also shown.}
...
As previously demonstrated (in Figure~\ref{fig:plate_conv_singularity}) modelling singularity can significantly improve the accuracy of the configurational forces at the crack front. In the next example we considered bone with heterogeneous density distribution mapped from CT scan data. 
In the Figure~\ref{fig:crack_singular_vs_non} results from crack analysis with and without Quarter Point elements are depicted. It is evident that accurate stress state at the tip has a negligible impact on the full crack propagation analysis and the resulting load factor.  
...
\begin{minipage}{.45\textwidth}
	...
	\node at (2,0.35) {	\includegraphics[width=3.5cm]{Figures/pngs/crack_surf_heter_compar.png}};
	\end{tikzpicture}
	\caption{Load factor versus crack area for heterogeneous versus homogeneous bone.}
	\label{fig:crack_homo_vs_het}
\end{minipage}%
\end{figure}
From the load-crack area curves in Figure~\ref{fig:crack_homo_vs_het} it can be observed that including density data from CT scans have a significant impact on the predicted load factor and crack path as well. 
...
Supplementary data (ct scans, meshfiles, command lines) necessary to reproduce results demonstrated in this work can be found in~\citep{karol_lewandowski_2019_dataset}. Presented bone remodelling and fracture mechanics approaches are submodules in MoFEM library, which can be easily installed using multiplatform Spack package manager~\citep{spack2015}. 
